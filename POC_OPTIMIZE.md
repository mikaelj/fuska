# Revised Plan: Optimize fuska-plan.md (Final)

**Goal:** Reduce `fuska-plan.md` token consumption by ~36-43% while improving maintainability.
...
## Token Economics: Honest Numbers

### Current State

| Component | Lines | Tokens (approx) |
|-----------|-------|-----------------|
| fuska-plan.md body | 1,447 | ~3,600 |
| Existing includes | 280 | ~700 |
| **Total prompt** | **1,727** | **~4,300** |

### After Optimization

| Component | Lines | Tokens (approx) |
|-----------|-------|-----------------|
| fuska-plan.md body | ~410 | ~1,000 |
| Existing includes | 280 | ~700 |
| New includes | 260 | ~650 |
| **Total prompt** | **~950** | **~2,400** |

**Actual reduction: ~36-43%** (not 69% as originally claimed)

### Why the Original Math Was Wrong

The original plan assumed `@include` directives don't count toward tokens. They do — all included content is expanded into the prompt before the LLM processes it.
...
|----------|-------|-------|
| Minimal | `--skip-research --skip-verify --no-review` | Core flow only, fastest |
| Standard | `--skip-verify --no-review` | Includes research spawn |
| Full | (no flags) | Complete flow with checker and review |

**Recommendation:** Use minimal scenario for token comparison (faster, isolates command overhead). Use full scenario for functional verification.

---

## Decisions Incorporated

1. **megamemory-quick-ref.md stays at 20 lines** — No risk of breaking functionality by being too terse.

2. **Scope limited to fuska-plan.md only** — Other commands not touched in this PoC.

3. **Markdown format** — Works for both OpenCode and Claude. No TypeScript config files needed.

---

## Files to Create (4 files)

### 1. `provider/opinkode/fuska/references/megamemory-quick-ref.md` (20 lines)
...
**megamemory:understand** returns `{concepts: [{id, name, kind, summary, children, edges}]}`
...
### 3. `provider/opinkode/fuska/templates/plan-prompts.md` (100 lines)
...
### 4. `provider/opinkode/fuska/templates/review-loop.md` (80 lines)
...
| Steps 8-9 | 60 | 40 | 20 |
| Step 10 | 50 | 15 | 35 |
| Steps 11-12 | 70 | 45 | 25 |
| Step 13 | 282 | 20 | 262 |
| Steps 14-15 | 50 | 35 | 15 |
| Offer next + success | 60 | 60 | 0 |
| **fuska-plan.md total** | **1,447** | **~410** | **~1,037** |
...
| Expected token reduction | ~36-43% |
