...
<required_reading>
@~/.config/opencode/get-shit-done-mm/references/verification-patterns.md
@~/.config/opencode/get-shit-done-mm/templates/verification-report.md
</required_reading>
...
This workflow stores verification results as concepts in the project knowledge graph:

### Concept: Phase {N} Verification
- **Kind:** `config`
- **Summary:** Verification results for phase N, including truth status, artifact checks, wiring verification, gaps, and recommendations
- **Parent:** Phase {N} plan concept
- **Edges:** `connects_to` to phase plan, `depends_on` on must-have artifacts

### Concept: Phase {N} Must-Haves
- **Kind:** `config`
- **Summary:** Must-have truths, artifacts, and key_links for phase N verification
- **Parent:** Phase {N} plan concept
- **Edges:** `connects_to` to phase goal

### Query Strategy
- Load phase plans: `megamemory:understand({query: "phase {N} plans"})`
- Extract must_haves from plan concept summaries
- Create verification results concept with all findings
- Update phase plan status based on verification outcome

### No File Creation
- Do NOT create VERIFICATION.md files
- Store all verification data in MegaMemory concepts
- Phase status lives in MegaMemory, not filesystem
</megamemory_guide>
