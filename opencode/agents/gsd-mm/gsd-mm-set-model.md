...
<role>
You are executing the `/gsd-mm-set-model` command. Configure the models assigned to each stage (planning, execution, verification) for a specific profile preset.
...
1. No args (interactive): `/gsd-mm-set-model`
2. Positional: `/gsd-mm-set-model quality` or `balanced` or `budget`
...
| Verification | gsd-verifier, gsd-integration-checker, gsd-set-profile, gsd-settings, gsd-mm-set-model |

**Profile presets:** Stored in MegaMemory `config` concept under `profiles.presets.{profile}`. Each profile has three keys: `planning`, `execution`, `verification`.
</context>
...
**Case A: No config concept found**
- Print: `Error: No GSD project found. Run /gsd-mm-new-project first.`
- Stop.

**Case B: Config exists but no `profiles.presets` key**
- Print: `Error: No model presets configured. Run /gsd-mm-settings first to initialize your profiles.`
- Stop.

**Case C: Config exists with `profiles.presets` key**
```typescript
...
- If user typed `/gsd-mm-set-model quality` (or balanced/budget), use that as `targetProfile`
...
```json
{
  ...
  "agent": {
    ...
    "gsd-mm-set-model": { "model": "{effective.verification}" }
  }
}
```
...
```text
To use this profile, run: /gsd-mm-set-profile {targetProfile}
```
...
- This command modifies **presets**, not overrides. Use `/gsd-mm-settings` for per-stage overrides.
