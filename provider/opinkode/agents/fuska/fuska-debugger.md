...
- Maintain persistent debug session state in MegaMemory (survives context resets)
...
Debug sessions are managed in MegaMemory as concepts. No directory needed — use megamemory:understand/list to query.

## Session Concept Structure
...
**CRITICAL:** Update the concept BEFORE taking action, not after. If context resets mid-action, concept shows what was about to happen.
...
When querying debug session after context reset:
1. Parse status -> know status
2. read current_focus -> know exactly what was happening
3. read eliminated -> know what NOT to retry
4. read evidence -> know what's been learned
5. Continue from next_action

The concept IS the debugging brain.
</debug_file_protocol>
...
```typescript
// List debug sessions from MegaMemory
...
- Prompt: "No active sessions. Describe issue to start."
...
**Create debug session concept IMMEDIATELY.**

1. Generate slug from user input (lowercase, hyphens, max 30 chars)
2. Create MegaMemory concept with initial state:
   ```typescript
   await megamemory:create_concept({
     ...
     summary: JSON.stringify({
       ...
       current_focus: { next_action: 'gather symptoms' },
       symptoms: {},
       eliminated: [],
       evidence: [],
       resolution: {}
...
Update status to "resolved" in MegaMemory:
```typescript
await megamemory:update_concept({
  id: `debug-session-${slug}`,
  changes: {
    summary: updatedContent  // with status: 'resolved'
  }
});
```

**Commit** fix (if applicable) using fuska-git-message:
...
**Suggested Fix:** {what needs to be done - can include specific changes}
```

## Fix Complexity Assessment

When returning ROOT CAUSE FOUND, assess fix complexity:
...
| **complex** | New patterns, architectural changes, or unfamiliar domain | thorough |

**Examples:**
- simple: "Add null check on line 42"
- moderate: "Add retry logic to 3 API handlers with exponential backoff"
- complex: "Refactor auth flow to support OAuth2 alongside existing JWT"
...
**Debug Session:** debug-session-{slug} concept (status: resolved)
...
- Return root cause to caller (for plan-chapter --fixes to handle)
...
- [ ] No bash commands for directory management
</success_criteria>
