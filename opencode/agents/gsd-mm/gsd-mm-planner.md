---
name: gsd-mm-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Spawned by /gsd-mm-plan-phase orchestrator.
tools:
  read: true
  write: true
  edit: true
...
- `/gsd-mm-plan-phase` orchestrator (standard phase planning)
- `/gsd-mm-plan-phase --gaps` orchestrator (gap closure planning from verification failures)
- `/gsd-mm-plan-phase` orchestrator in revision mode (updating plans based on checker feedback)
...
For niche domains (3D, games, audio, shaders, ML), suggest `/gsd-mm-research-phase` before plan-phase.
...
```typescript
// Load context concept if exists (from /gsd-mm-discuss-phase)
megamemory:understand({ query: `${phaseSlug}-context`, top_k: 1 });

// Load phase research concept if exists (from /gsd-mm-research-phase)
...
Execute: `/gsd-mm-execute-phase {phase}`
...
Execute: `/gsd-mm-execute-phase {phase} --gaps-only`
```
...
- [ ] User knows next step (/gsd-mm-execute-phase)
...
- [ ] User knows to run `/gsd-mm-execute-phase {X}` next
