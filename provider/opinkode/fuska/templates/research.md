# Research Template (MegaMemory-Backed)
...
- `standard_stack` (object)
  - `core` (array) - Core libraries with versions and rationale
  - `supporting` (array) - Supporting libraries with use cases
  - `alternatives` (array) - Alternatives considered with tradeoffs
  - `installation` (string) - Installation commands
- `architecture_patterns` (object)
  - `project_structure` (string) - Recommended folder layout
  - `patterns` (array) - Named patterns with descriptions, conditions, examples
  - `anti_patterns` (array) - Anti-patterns with rationale
- `dont_hand_roll` (object)
  - `problems` (array) - Problems with existing solutions
  - `key_insight` (string) - Why custom solutions are worse
- `common_pitfalls` (array) - Named pitfalls with description, root cause, prevention, warning signs
- `code_examples` (array) - Verified code examples with sources
- `sota_updates` (object)
  - `approaches_changed` (array) - Old → New transitions
  - `new_tools` (array) - New tools/patterns to consider
  - `deprecated` (array) - Deprecated approaches
- `open_questions` (array) - Unresolved questions with partial info, gaps, recommendations
- `sources` (object)
  - `primary` (array) - Context7 library IDs, official docs (HIGH confidence)
  - `secondary` (array) - Verified webfetch sources (MEDIUM confidence)
  - `tertiary` (array) - Unverified webfetch (LOW confidence)
- `metadata` (object)
  - `research_scope` (object) - Core technology, ecosystem, patterns, pitfalls
  - `confidence_breakdown` (object) - Confidence by section
  - `research_date` (string) - ISO timestamp
...
// Returns research concepts with:
// - Standard stack (libraries, versions, why)
// - Architecture patterns (with code examples)
// - Common pitfalls and how to avoid
// Used when planning similar features in same domain
```

## Query Standard Stack by Library
...
// Returns research with pitfalls about that pattern:
// - What goes wrong
// - Why it happens (root cause)
// - How to avoid
// - Warning signs for early detection
// Used in verification criteria and task planning
```
...
## Example 2: Querying Auth Research Patterns
...
// Returns:
// - Standard stack: jose, prisma, bcrypt
// - Pattern 1: Refresh rotation on each request
// - Pattern 2: httpOnly cookies, not localStorage
// - Pitfall 1: Storing tokens in localStorage (XSS risk)
// - Pitfall 2: Missing password hashing (critical security)
// Code examples: token rotation, protected routes
```

## Example 3: Querying Prisma Patterns
...
// Returns all Prisma research with:
// - Schema patterns: model structure, relations, indexes
// - Migration patterns: when to migrate, how to handle breaking changes
// - Query patterns: where vs findMany, include vs select, transaction patterns
// - Code examples from official docs
// - Pitfalls: N+1 queries, missing indexes, circular relations
```

## Example 4: Querying Performance Pitfalls
...
// Returns:
// - Three.js: InstancedMesh anti-pattern, draw call death
// - React: Unnecessary re-renders, missing memo, large bundles
// - Database: N+1 queries, missing indexes, circular joins
// Warning signs enable early detection during implementation
```
...
**Structure:**
- Use XML tags for section markers (matches Fuska templates)
