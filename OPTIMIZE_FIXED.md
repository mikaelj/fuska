# Optimization Plan: Remove Non-Essential Tool Calls (Revised)
...
| `agents/fuska/fuska-executor.md` | Same bash time commands (lines 81-82); `duration_minutes`, `started`, `completed` from summary |
...
| File | What to Change |
|------|----------------|
| All commands in Section 6 | Remove `last_activity` field from state updates |
...
| `fuska/scripts/state-ops.ts` | Remove `last_activity` from state update functions |
...
| `templates/UAT.md` | `started` field (lines 19, 241, 307, 415) | Non-essential timing |
| `templates/DEBUG.md` | `started` field (lines 32, 180, 199, 366) | Non-essential timing |
...
- `fuska/workflows/transition.md` — Remove `last_activity` from JSON examples (lines 367, 379, 393, 408)

### 9. Script files (remove `last_activity`):
- `fuska/scripts/state-ops.ts` — Remove `last_activity` from update functions (lines 31, 39, 55, 70)
- `fuska/scripts/validators.ts` — Remove `last_activity` validation (lines 98-99)

### 10. Template files (remove timing fields):
- `fuska/templates/UAT.md` — Remove `started` field
- `fuska/templates/DEBUG.md` — Remove `started` field

### 11. Test files (update to match):
...
- `fuska/scripts/__tests__/project-templates.test.ts`

### 12. Migration file (keep for backward compatibility):
- `fuska/migration/enhanced-migration.ts` — **Keep** `last_activity` handling to support importing legacy projects. Old data may have this field; migration should preserve it but new code won't use it.
...
| Change | Reason |
|--------|--------|
| Added `fuska-import.md` to Section 6 | Found `last_activity` update at line 268 |
| Removed `fuska-resume-work.md` from Section 7 | Duplicate — already in Section 6 |
| Added `transition.md` to Section 8 | Contains `last_activity` in examples |
| Added `state-ops.ts` to new Section 9 | Contains `last_activity` updates |
| Added `validators.ts` to Section 9 | Contains `last_activity` validation |
| Added `UAT.md` and `DEBUG.md` to Section 10 | Remove `started` field per user clarification |
| Added `project-templates.test.ts` to Section 11 | Found during validation |
| Added Section 12 for migration | Keep for backward compatibility |
