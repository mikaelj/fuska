import { PhaseConceptTemplates } from '../phase-templates';

describe('PhaseConceptTemplates', () => {
  describe('createContext', () => {
    it('creates phase context concept', () => {
      ...
      const concept = PhaseConceptTemplates.createContext('phase-01', contextData, []);

      expect(concept.name).toBe('phase-01-context');
      expect(concept.kind).toBe('config');
      expect(concept.summary).toContain('"gathered":"2025-01-20"');
      expect(concept.summary).toContain('"phase_boundary":"Implement JWT authentication"');
      expect(concept.summary).toContain('"auth_type":"JWT"');
      expect(concept.summary).toContain('<domain>');
      expect(concept.summary).toContain('## Phase Boundary');
      ...
      expect(concept.parent_id).toBe('phase-01');
      expect(concept.edges).toEqual([{ to: 'phase-01', relation: 'configured_by' }]);
    });
    ...
    it('includes relevant knowledge when provided', () => {
      ...
      const concept = PhaseConceptTemplates.createContext('phase-01', contextData, relevantKnowledge);
  ...
  describe('createPlan', () => {
    it('creates phase plan concept', () => {
      ...
      const concept = PhaseConceptTemplates.createPlan('phase-01', 1, planData, patterns, []);

      expect(concept.name).toBe('phase-01-plan-1');
      ...
      expect(concept.edges[0]).toEqual({ to: 'phase-01', relation: 'implements' });
    ...
    it('handles plan without megamemory references', () => {
      ...
      const concept = PhaseConceptTemplates.createPlan('phase-01', 1, planData, [], []);
  ...
  describe('createResearch', () => {
    it('creates research concept', () => {
      ...
      const concept = PhaseConceptTemplates.createResearch('phase-01', researchData);

      expect(concept.name).toBe('phase-01-research');
      ...
      expect(concept.parent_id).toBe('phase-01');
      expect(concept.edges).toEqual([{ to: 'phase-01', relation: 'connects_to' }]);
    });
  ...
  describe('createSummary', () => {
    it('creates summary concept', () => {
      const summaryData = {
        ...
        deviations: [],
        issues_encountered: [],
        next_phase_readiness: 'Ready - all must-haves delivered'
      };

      const concept = PhaseConceptTemplates.createSummary('phase-01', 1, summaryData);

      expect(concept.name).toBe('phase-01-plan-1-summary');
      expect(concept.kind).toBe('component');
      expect(concept.summary).toContain('"phase":"phase-01"');
      expect(concept.summary).toContain('"plan":"phase-01-plan-1"');
      ...
      expect(concept.summary).toContain('phase-01');
      ...
      expect(concept.edges[0]).toEqual({ to: 'phase-01-plan-1', relation: 'connects_to' });
      expect(concept.edges[1]).toEqual({ to: 'phase-01', relation: 'connects_to' });
      expect(concept.created_by_task).toBe('phase-01-plan-1');
    });
  });
  ...
  describe('createUAT', () => {
    it('creates UAT concept', () => {
      const uatData = {
        ...
        concepts_reviewed: ['phase-01-context', 'phase-01-plan-1', 'phase-01-plan-1-summary']
      };

      const concept = PhaseConceptTemplates.createUAT('phase-01', uatData);

      expect(concept.name).toBe('phase-01-uat');
      expect(concept.kind).toBe('component');
      ...
      expect(concept.summary).toContain('phase-01-context');
      expect(concept.parent_id).toBe('phase-01');
      expect(concept.edges).toHaveLength(4);
      expect(concept.edges[0]).toEqual({ to: 'phase-01', relation: 'connects_to' });
      expect(concept.edges[1]).toEqual({ to: 'phase-01-context', relation: 'connects_to' });
      expect(concept.edges[2]).toEqual({ to: 'phase-01-plan-1', relation: 'connects_to' });
      expect(concept.edges[3]).toEqual({ to: 'phase-01-plan-1-summary', relation: 'connects_to' });
    });
    ...
    it('handles UAT with no issues', () => {
      const uatData = {
        ...
        issues_found: [],
        recommendations: [],
        concepts_reviewed: ['phase-01-context']
      };
