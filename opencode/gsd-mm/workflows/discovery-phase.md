...
NOTE: For comprehensive ecosystem research ("how do experts build this"), use /gsd-research-phase instead, which creates a RESEARCH concept.
</purpose>
...
<megamemory_guide>
**MegaMemory Strategy for Discovery:**

**When to Query:**
- At start: `megamemory:understand` with query about current phase/topic to load existing context
- Use results to avoid redundant research

**When to Create:**
- After Level 2-3 discovery: `megamemory:create_concept` to store findings
- Concept type: `feature` (for feature-specific discovery) or `component` (for library/framework)
- Summary must be JSON-parseable with structured findings
...
```json
{
  ...
  "findings": {
    "options": [
      {"name": "Option A", "pros": [], "cons": [], "confidence": "high"},
      {"name": "Option B", "pros": [], "cons": [], "confidence": "medium"}
    ],
    "key_decisions": [],
    "open_questions": [],
    "assumptions": []
  },
  "sources": ["Context7", "official docs URL", "webfetch URL"],
...
**Edges:**
- `depends_on`: parent phase or prerequisites
- `configured_by`: config concepts (if applicable)

**Parsing:**
When retrieving discovery results later, parse `concept.summary` as JSON to access structured data.
</megamemory_guide>
...
```
megamemory:understand with query: "discovery [phase] [topic] findings research"
```
...
```
Discovery complete: MM concept "[Phase] [topic] discovery"
