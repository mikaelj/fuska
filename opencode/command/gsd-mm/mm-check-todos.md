...
→ Suggest: "Add todos during work sessions with /gsd-mm-add-todo"
→ Stop
...
Todos are captured during work sessions with /gsd-mm-add-todo.

────────────────────────────────────────────────────────────

Would you like to:

1. Continue with current phase (/gsd-mm-progress)
2. Add a todo now (/gsd-mm-add-todo)
```
...
```
const areaFilter = $ARGUMENTS.trim() // e.g., "api" or ""
```

- `/gsd-mm-check-todos` → show all (areaFilter = "")
- `/gsd-mm-check-todos api` → filter to area:api only
...
- /gsd-mm-check-todos [area] to filter by area
- q to exit
```
...
Display: `/gsd-mm-add-phase ${selectedTodo.title}`
...
- Don't create plans from this command — route to /gsd-mm-plan-phase or /gsd-mm-add-phase
