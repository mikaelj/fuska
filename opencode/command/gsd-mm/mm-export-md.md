...
```
const outputDir = $ARGUMENTS.trim() || ".planning.export"
```
...
Confirm: "✓ REQUIREMENTS.md exported (${allRequirements.length} requirements)"
...
Confirm: "✓ ROADMAP.md exported (${phases.length} phases)"
...
Confirm: "✓ MILESTONES.md exported (${milestones.length} milestones)"
...
Confirm: "✓ Phase ${phaseSlug} exported (context: ${phaseGroup.context.length}, plans: ${phaseGroup.plans.length}, summaries: ${phaseGroup.summaries.length}, research: ${phaseGroup.research.length}, uat: ${phaseGroup.uat.length})"
...
Confirm: "✓ Todos exported (${todos.length} total)"
...
**Re-export with fresh data:**
  /gsd-mm-export-md [other-directory]
