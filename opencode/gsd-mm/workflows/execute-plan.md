...
## MegaMemory Response Parsing

MegaMemory returns structured concept data that must be parsed for use in workflow orchestration.
  ...
  "concepts": [
    {
      ...
      "summary": "JSON-serializable data string",
      "why": "Rationale",
      "file_refs": ["file-path:line-range", ...],
      "edges": [
        {"to": "other-concept-id", "relation": "depends_on|implements|calls|connects_to|configured_by", "description": "..."},
        ...
      ],
      "created_by_task": "...",
      "children": [...],
...
### Parsing JSON from concept.summary

Many concepts store structured data in `summary` field as JSON string:

```javascript
// Example: State concept summary
{
  "current_phase": "01",
  "current_plan": "01",
  "status": "in_progress",
  "decisions": [
    {"phase": "01", "decision": "Use Next.js", "rationale": "..."},
    ...
  ],
  "blockers": [...],
  "progress_percent": 40
...
```javascript
// When you get megamemory:understand response:
const stateConcept = concepts.find(c => c.name === "Project State");
...
const configConcept = concepts.find(c => c.name === "Planning Config");

if (!configConcept) {
  // Concept doesn't exist - use defaults
  const modelProfile = "balanced";
} else {
  const configData = JSON.parse(configConcept.summary);
...
| What you need | Query to use | Expected result |
|---------------|--------------|-----------------|
| Project state | `megamemory:understand({query: "project state"})` | Concept with "Project State" name |
| Phase plan | `megamemory:understand({query: "phase 01 plan"})` | Concept with plan data in summary |
| Phase summary | `megamemory:understand({query: "phase 01 summary"})` | Concept with summary data |
| Roadmap | `megamemory:understand({query: "roadmap"})` | Concept with roadmap data |
| Config | `megamemory:understand({query: "planning config"})` | Concept with config data |

### Creating/Updating Concepts
...
megamemory:create_concept({
  ...
  why: "Captures execution results for plan 01-01",
  parent_id: "phase-01-concept-id",
  edges: [
    {to: "project-state-id", relation: "configured_by", description: "Updates project progress"},
    {to: "phase-00-summary-id", relation: "depends_on", description: "Built on previous phase"}
  ],
...
```javascript
megamemory:link({
  from: "01-01-summary-id",
  to: "01-02-plan-id",
  relation: "provides",
  description: "Foundation for next plan in phase"
})
```

### Agent Tracking

Agent spawning and resume detection uses a MegaMemory `agent-tracking` concept (kind: `config`).
...
```javascript
{
  version: "1.0",
  max_entries: 50,
  current_agent_id: null,  // Currently active agent ID, or null
  agent_history: [
    {
      agent_id: "uuid",
      task_description: "Execute tasks [X-Y] from plan 01-01",
      phase: "01",
      plan: "01",
      segment: null,           // null for Pattern A, number for Pattern B
      timestamp: "ISO-8601",
      status: "spawned",       // "spawned" | "completed"
...
**Parse response:**
- If config concept exists: Parse `summary` JSON and extract `model_profile`
