...
<megamemory_guide>
**MegaMemory Strategy for Diagnosis:**

**When to Query:**
- At start: `megamemory:understand` with query about current phase, gaps, blockers, or diagnosis history
- Load existing diagnoses to avoid redundant work
- Check for related concepts (features, components with known issues)

**When to Create:**
- After collecting all root causes: `megamemory:create_concept` to store diagnosis
- Concept type: `feature` (diagnosis of a feature) or `component` (diagnosis of a component)
- Summary must be JSON-parseable with structured diagnosis data

**Concept Structure:**
```json
{
  "phase": "XX",
  "gap_name": "what gap was diagnosed",
  ...
  "suggested_fix": "brief hint for plan-phase --gaps",
...
**Edges:**
- `depends_on`: parent phase concept
- `connects_to`: feature or component being diagnosed

**Multiple Gaps:**
Create one concept per gap with clear naming: "[Phase] [gap-slug] diagnosis"

**Parsing:**
When retrieving diagnosis results, parse `concept.summary` as JSON to access:
- root_cause
- evidence list
- suggested_fix
- missing actions
...
Debug sessions are stored as MegaMemory concepts (kind: "component") instead of .planning/debug/ files:

```
megamemory:understand({query: "debug {slug}"})
// Access via: result.concepts[0]
// Parse:  JSON.parse(result.concepts[0].summary)
```

**Debug concept structure:**
```json
{
  "slug": "issue-slug",
  "timestamp": "ISO-8601",
  ...
  "investigation": "hypothesis and findings",
  "resolution": "root cause or null",
  "status": "investigating|root_cause_found|completed"
}
```
</megamemory_guide>
