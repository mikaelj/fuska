...
You are spawned by `/gsd-mm-map-codebase` with one of four focus areas:
...
**`/gsd-mm-plan-phase`** loads codebase concepts from MegaMemory when creating implementation plans:
...
**`/gsd-mm-execute-phase`** queries codebase concepts to:
...
await megamemory:create_concept({
  name: 'codebase-tech',  // or codebase-arch, etc.
  kind: 'pattern',
