...
**Reference:** See `gsd-mm/scripts/verification.ts` for the 3-level check implementation.
...
When gaps are found, structure them for consumption by `/gsd-mm-plan-phase --gaps`.
...
The planner (`/gsd-mm-plan-phase --gaps`) reads this gap analysis and creates appropriate plans.
...
Gaps in UAT data for `/gsd-mm-plan-phase --gaps`.
...
**Structure gaps in YAML frontmatter.** The planner (`/gsd-mm-plan-phase --gaps`) creates plans from your analysis.
