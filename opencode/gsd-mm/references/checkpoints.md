...
## Concept Schema: Checkpoint Patterns

Checkpoints are interaction points where human verification or decisions are needed during autonomous plan execution.

### Concept Types

**1. `checkpoint:pattern`**
```typescript
interface CheckpointPatternConcept {
  kind: "feature" | "pattern";
  name: string;  // "human-verify", "decision", "human-action"
  summary: string;
  why: string;
  file_refs: string[];
  edges: Array<{
...
**2. `checkpoint:rule`**
```typescript
interface CheckpointRuleConcept {
  kind: "config" | "decision";
  name: string;
  summary: string;  // "If OpenCode can run it, OpenCode runs it"
  why: string;
  file_refs: string[];
  examples: string[];
}
...
**3. `checkpoint:example`**
```typescript
interface CheckpointExampleConcept {
  kind: "component" | "feature";
  name: string;  // "Vercel Deployment Verification"
  summary: string;
  checkpoint_type: "human-verify" | "decision" | "human-action";
  setup_automation: string[];  // OpenCode automation steps before checkpoint
  verification_steps: string[];  // Human verification steps
  file_refs: string[];
}
```
...
All checkpoint concepts must:
- Define `why` rationale (not just what)
- Include concrete file references (line ranges where applicable)
- Link to related patterns via `edges`
...
### 1. Creating Checkpoint Patterns

```typescript
// Create a checkpoint pattern definition
await megamemory_create_concept({
  name: "checkpoint:human-verify",
  kind: "pattern",
  summary: "Most common checkpoint type (90% of checkpoints). OpenCode completes automated work (builds, deploys, starts servers), then human confirms visual/functional correctness. Used for: UI layout checks, interactive flows, accessibility testing, animation smoothness. OpenCode ALWAYS starts dev servers and sets up environment before checkpoint - user only visits URLs and verifies.",
  why: "Visual and functional verification requires human judgment that cannot be automated. This pattern ensures OpenCode automates everything automatable first, only asking human for unavoidable verification steps.",
  file_refs: [
    "get-shit-done-mm/references/checkpoints.md:1-120"
  ],
  edges: [{
    to: "rule:automate-everything",
    relation: "depends_on",
    description: "human-verify requires OpenCode to automate all setup work before verification"
  }]
});

// Create a golden rule
await megamemory_create_concept({
  name: "rule:automate-everything",
  kind: "decision",
  summary: "If OpenCode can run it via CLI/API, OpenCode MUST run it. Never ask user to execute CLI commands, start servers, or run builds. OpenCode sets up the verification environment (start dev servers, seed databases, configure env vars). User only does what requires human judgment (visual checks, UX evaluation, 'does this feel right?').",
  why: "Maximizes automation and reduces friction. Checkpoints exist only for verification and decisions, not manual work.",
  file_refs: ["get-shit-done-mm/references/checkpoints.md:6-11"],
  examples: [
    "OpenCode runs `vercel --yes` to deploy, user only visits URL to verify",
    "OpenCode runs `npm run dev` to start server, user only visits localhost to verify",
    "OpenCode uses Stripe API to create webhook, user only verifies in dashboard"
  ]
});
```

### 2. Querying Checkpoint Knowledge

```typescript
// Before creating a checkpoint, query pattern library
const patterns = await megamemory_understand({
  query: "checkpoint patterns for verifying deployments after automation",
  top_k: 5
});

// Returns relevant patterns with examples:
// - checkpoint:human-verify with deployment verification examples
// - automation setup steps (vercel CLI commands)
// - verification protocols (what to check in browser)

// Query for automation rules
const automationRules = await megamemory_understand({
  query: "what can OpenCode automate via CLI API",
  top_k: 10
});

// Returns rules about:
// - Deployment automation (vercel, fly, railway)
// - Dev server automation (npm run dev, vite, etc.)
// - Environment variable automation (CLI commands vs dashboard)
// - When to use human-action checkpoints (auth gates only)
```

### 3. Updating and Linking Concepts

```typescript
// After implementing a checkpoint pattern, link to specific implementation
await megamemory_link({
  from: "checkpoint:human-verify",
  to: "example:vercel-deployment-verification",
  relation: "implements",
  description: "Vercel deployment verification follows human-verify pattern: OpenCode deploys via CLI, human verifies in browser"
});

// Update pattern with new learned example
await megamemory_update_concept({
  id: "checkpoint:human-verify",
  changes: {
    summary: "Most common checkpoint type (90% of checkpoints). OpenCode completes automated work, then human confirms correctness. NEW: Also适用于Xcode builds - verify .app launches and functions correctly."
  }
});
```

### 4. Recording Auth Gates as Dynamic Patterns

```typescript
// When encountering auth error, record it as an auth gate pattern
await megamemory_create_concept({
  name: "auth-gate:vercel-cli",
  kind: "pattern",
  summary: "Dynamic checkpoint created when Vercel CLI returns 'Not authenticated' error. Pattern: OpenCode tries automation → auth error → creates checkpoint:human-action → user runs `vercel login` → OpenCode retries → continues. NOT a failure - expected gate requiring credentials to unblock automation.",
  why: "Missing auth is expected during automation. Auth gates are created dynamically, not pre-planned. They unblock automation rather than representing pre-planned manual work.",
  file_refs: ["get-shit-done-mm/references/checkpoints.md:294-477"],
  edges: [{
    to: "checkpoint:human-action",
    relation: "implements",
    description: "Auth gates are a specialized form of checkpoint:human-action created dynamically"
  }]
});
```
...
// - Automation: "Run `vercel --yes`, capture URL from output"
// - Verification: "Visit URL, check homepage loads, no console errors"
...
const deploymentUrl = extractUrlFromOutput(deployResult);

// 2. Record checkpoint concept
await megamemory_create_concept({
  name: "checkpoint:verify-deployment-current-task",
  kind: "feature",
  summary: `Deployment checkpoint for current task: Deployed to ${deploymentUrl}. User must visit URL and verify homepage loads without errors. OpenCode already automated deployment via vercel CLI.`,
  why: "Verification requires human judgment - visual and functional testing cannot be automated.",
  file_refs: ["plan:current-task"],
  checkpoint_type: "human-verify",
  setup_automation: [
    "Run `vercel --yes` to deploy",
    "Capture deployment URL from output",
    "Verify deployment exists via `vercel ls`"
  ],
  verification_steps: [
    `Visit ${deploymentUrl}`,
...
Built: Deployed to ${deploymentUrl}

How to verify:
  1. Visit: ${deploymentUrl}
...
// 4. Wait for user response, then resume
```

## Example 2: Decision Checkpoint with Options

```typescript
// Agent needs to make a technology decision
const decisionPattern = await megamemory_understand({
  query: "checkpoint:decision structure for technology selection with pros and cons"
});

// Agent creates decision checkpoint
await megamemory_create_concept({
  name: "decision:auth-provider-selection",
  kind: "feature",
  summary: "Authentication provider decision with three options: Supabase (built-in auth, free tier), Clerk (best DX, paid after 10k MAU), NextAuth (self-hosted, free). Context: App needs user authentication, using Supabase DB already.",
  why: "Technology selection affects implementation direction. Each option has different tradeoffs that human must evaluate.",
  file_refs: ["plan:auth-implementation"],
  checkpoint_type: "decision",
  options: [
    {
      id: "supabase",
      name: "Supabase Auth",
      pros: "Built-in with Supabase DB we're using, generous free tier, row-level security integration",
      cons: "Less customizable UI, tied to Supabase ecosystem"
    },
    {
      id: "clerk",
      name: "Clerk",
      pros: "Beautiful pre-built UI, best developer experience, excellent docs",
      cons: "Paid after 10k MAU, vendor lock-in"
    },
    {
      id: "nextauth",
      name: "NextAuth.js",
      pros: "Free, self-hosted, maximum control, widely adopted",
      cons: "More setup work, you manage security updates, UI is DIY"
    }
  ]
});
...
Context: Need user authentication. Using Supabase DB already.
...
## Example 3: Auth Gate Handling

```typescript
// Agent tries to automate but hits auth error
try {
  ...
  if (error.message.includes("Not authenticated")) {
    // This is NOT a failure - it's an auth gate
    await megamemory_create_concept({
      name: "auth-gate:vercel-cli-current-task",
      kind: "feature",
      summary: "Vercel CLI authentication gate encountered during deployment. Tried `vercel --yes` but got 'Not authenticated' error. Need user to run `vercel login` to authenticate, then retry deployment.",
      why: "Missing auth is expected. This is a gate, not a failure. Unblocks automation rather than representing pre-planned manual work.",
      file_refs: ["plan:current-task"],
      checkpoint_type: "human-action",
      authentication_service: "vercel",
      auth_command: "vercel login",
      retry_after_auth: true
    });

    // Present auth gate checkpoint
    console.log(`
╔═══════════════════════════════════════════════════════╗
...
`);
    ...
    // Wait for user to authenticate, then retry
    await verifyAndRetry("vercel whoami", "vercel --yes");
  }
}
```

## Example 4: Recording Anti-Patterns to Avoid

```typescript
// Agent records anti-patterns to avoid
await megamemory_create_concept({
  name: "anti-pattern:ask-user-to-start-server",
  kind: "pattern",
  summary: "WRONG: Asking user to run `npm run dev` in checkpoint. User should only visit URLs, not execute CLI commands. CORRECT: OpenCode starts server before checkpoint (`npm run dev &`, verify with curl), user only visits URL to verify.",
  why: "Maximizes automation. OpenCode can start servers via CLI - this is automatable work that should never be in checkpoint instructions.",
  file_refs: ["get-shit-done-mm/references/checkpoints.md:854-886"],
  edges: [{
    to: "rule:automate-everything",
    relation: "configured_by",
    description: "Anti-pattern violates the rule that OpenCode automates everything possible"
  }],
  wrong_example: `<task type="checkpoint:human-verify">
  ...
  correct_example: `<task type="auto">
...
<task type="checkpoint:human-verify">
  ...
  <how-to-verify>Visit http://localhost:3000/dashboard and verify layout matches design</how-to-verify>
</task>`
});
...
// Returns anti-patterns like "ask-user-to-start-server", "ask-user-to-add-env-vars"
// Agent ensures checkpoint doesn't violate these patterns
```

## Example 5: Linking Related Concepts

```typescript
// After creating a deployment verification, link to automation rules
const deploymentCheckpoint = await megamemory_create_concept({
  name: "checkpoint:verify-vercel-deployment-task-123",
  kind: "feature",
...
await megamemory_link({
  from: "checkpoint:verify-vercel-deployment-task-123",
  to: "checkpoint:human-verify",
  relation: "implements",
  description: "This verification follows the human-verify checkpoint pattern"
});

// Link to automation rules used
await megamemory_link({
  from: "checkpoint:verify-vercel-deployment-task-123",
  to: "rule:automate-everything",
  relation: "depends_on",
  description: "This checkpoint depends on OpenCode automating deployment via vercel CLI"
});

// Query all human-verify checkpoints for current project
const allVerifyCheckpoints = await megamemory_understand({
  query: "all human-verify checkpoints in current project with their implementations"
});
// Returns concepts and edges showing the verification graph
```
