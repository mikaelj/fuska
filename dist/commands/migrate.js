...
class PlanningToMegaMemoryMigration {
    ...
    quoteEmbeddedDoubleQuotes(content) {
        let quoted = 0;
        const newContent = content.replace(/^(\s+-\s+)(.+)$/gm, (match, prefix, item) => {
    ...
    cleanYamlContent(content, filename) {
        try {
            ...
            try {
                (0, gray_matter_1.default)(cleaned);
                return cleaned;
            }
            catch (e2) {
    ...
    async createMegaMemoryClient() {
        ...
        const { understand, createConcept, updateConcept, link, removeConcept, listRoots } = await Promise.resolve().then(() => __importStar(require('megamemory/dist/tools.js')));
        ...
        this.megamemory = {
            ...
            async link(params) {
                link(db, { from: params.from, to: params.to, relation: params.relation });
                return { success: true };
            },
            async list_roots() {
    ...
    async readPlanningFiles() {
        ...
        if (await fs.pathExists(milestoneDir)) {
            const mfiles = await (0, glob_1.glob)('*ROADMAP.md', { cwd: milestoneDir, absolute: true });
        ...
        if (await fs.pathExists(researchDir)) {
            const researchFiles = await (0, glob_1.glob)('*.md', { cwd: researchDir, absolute: true });
        ...
        if (await fs.pathExists(todosDir)) {
            const todoFiles = await (0, glob_1.glob)('*.md', { cwd: todosDir, absolute: true });
    ...
    async readPhaseFiles(phasePath) {
        ...
        const allFiles = await (0, glob_1.glob)('*.md', { cwd: phasePath, absolute: true });
    ...
    async migrateProject(files) {
        ...
        const { ProjectConceptTemplates } = await Promise.resolve().then(() => __importStar(require('../scripts/project-templates')));
        ...
        if (files.requirements) {
            ...
            for (const req of requirements) {
                const concept = ProjectConceptTemplates.createRequirement(projectData.slug, req.id, req.description, req.status);
        ...
        for (const phase of allPhases.values()) {
            const concept = ProjectConceptTemplates.createPhase(projectData.slug, phase.number, phase.slug, phase.name, phase.goal);
            await this.createConcept(concept);
        }
        if (files.state) {
    ...
    async migratePhases(files) {
        ...
        const { ProjectConceptTemplates } = await Promise.resolve().then(() => __importStar(require('../scripts/project-templates')));
        const { PhaseConceptTemplates } = await Promise.resolve().then(() => __importStar(require('../scripts/phase-templates')));
        const { makeId } = await Promise.resolve().then(() => __importStar(require('megamemory/dist/tools.js')));
        const projectData = this.parseProjectFile(files.project, files.config);
        ...
        for (const [phaseDir, phaseFiles] of mergedPhases) {
            ...
            if (phaseFiles.research) {
                ...
                if (researchData && Object.keys(researchData).length > 0) {
                    ...
                    concept.parent_id = phaseParentId;
                    concept.edges = [{ to: phaseParentId, relation: 'connects_to' }];
                    await this.createConcept(concept);
                }
            }
            for (const [summaryNum, summaryContent] of phaseFiles.summaries) {
                ...
                if (summaryData && Object.keys(summaryData).length > 0) {
                    ...
                    concept.parent_id = phaseParentId;
                    concept.edges = [
                        { to: makeId(`${phaseName}-plan-${summaryNum}`, phaseParentId), relation: 'connects_to' },
                        { to: phaseParentId, relation: 'connects_to' }
                    ];
                    await this.createConcept(concept);
                }
            }
            if (phaseFiles.uat) {
                ...
                if (uatData) {
                    ...
                    concept.edges = [
                        { to: phaseParentId, relation: 'connects_to' },
                        ...uatData.concepts_reviewed.map((c) => ({ to: c, relation: 'connects_to' }))
                    ];
                    await this.createConcept(concept);
                }
            }
    ...
    async migrateResearch(files) {
        ...
        for (const [name, content] of files.research) {
            ...
            const concept = {
                ...
                edges: [{ to: researchParentId, relation: 'connects_to' }]
            };
            await this.createConcept(concept);
        }
