...
**Summary:** Executable phase plan with wave grouping, tasks, verification criteria, and goal-backward must-haves. Includes dependencies, file modifications, and autonomy flags for parallel execution.

**Fields:**
- `phase_id` (string) - Phase identifier (e.g., "03-features")
- `plan_id` (string) - Plan number within phase (e.g., "01", "02")
- `type` (string) - "execute" or "tdd"
- `wave` (number) - Execution wave number for parallel grouping
...
**Relationships:**
- `depends_on` → `phase-context:phase_id` - Decisions from context inform plan
- `depends_on` → `phase-research:phase_id` - Research findings inform plan
- `depends_on` → `phase-plan:depends_on` - Prior plans this depends on
- `connects_to` → `phase-summary:plan_id` - Creates summary on completion
...
```typescript
// When creating an executable plan for a phase
await megamemory.create_concept({
  name: `phase-plan:${phaseId}-${planId}`,
  kind: "feature",
  ...
  edges: [
    ...
    ...depends_on.map(depPlanId => ({
      to: `phase-plan:${depPlanId}`,
      relation: "depends_on",
      description: `Requires output from plan ${depPlanId}`
    })),
    {
      to: `phase-summary:${phaseId}-${planId}`,
...
```typescript
// Get all plans for a specific wave (parallel execution)
const wavePlans = await megamemory.understand({
  query: `phase plans wave ${waveNumber} for ${phaseId}`,
  top_k: 10
})

// Filter concepts to find wave N plans
const plansToExecute = wavePlans.concepts
  .filter(c => c.summary.includes(`Wave ${waveNumber}`))
  .map(c => ({
    id: c.name,
...
await megamemory.link({
  from: `phase-plan:${phaseId}-${planId}`,
  to: `phase-summary:${phaseId}-${planId}`,
  relation: "connects_to",
...
## Example 1: Creating Parallel Plan (Wave 1)

```typescript
// Plan 01 - User feature (can run in parallel with Product, Order)
await megamemory.create_concept({
  name: "phase-plan:03-features-01",
  kind: "feature",
  summary: "Plan 01 for phase 03-features: Implement complete User feature as vertical slice. Wave 1, depends on []. Tasks: 2. Autonomous: true. Must-haves: 3 truths, 3 artifacts.",
  why: "Self-contained user management that can run parallel to other features",
  parent_id: "project:myapp",
  edges: [
    {
      to: "phase-context:03-features",
      relation: "depends_on",
...
## Example 2: Querying Wave Plans for Parallel Execution

```typescript
// Execute-phase agent asks: what can I run in parallel now?
const currentWave = 1
const result = await megamemory.understand({
  query: `phase plans wave ${currentWave} that are autonomous`,
  top_k: 10
})

// Returns all Wave 1 autonomous plans that can execute simultaneously
// Plans: 03-01 (User), 03-02 (Product), 03-03 (Order)
// Execute-agent spawns 3 parallel subagents
```

## Example 3: Sequential Plan (Wave 2, depends on Wave 1)

```typescript
// Plan 04 - Dashboard (depends on User, Product being complete)
await megamemory.create_concept({
  name: "phase-plan:03-features-04",
  kind: "feature",
  summary: "Plan 04 for phase 03-features: Build dashboard integrating User and Product. Wave 2, depends on [03-01, 03-02, 03-03]. Tasks: 3. Autonomous: false. Must-haves: 2 truths, 2 artifacts.",
...
```typescript
// Verification agent asks: what must be TRUE for phase completion?
const result = await megamemory.understand({
  query: `phase ${phaseId} must-have verification criteria truths artifacts`,
  top_k: 5
})
...
```javascript
// Phase context (implementation decisions)
const contextResult = await megamemory.understand({query: `phase ${phaseId} context`})
const contextData = JSON.parse(contextResult.concepts[0].summary)

// Phase research (findings from investigation)
const researchResult = await megamemory.understand({query: `phase ${phaseId} research`})
const researchData = JSON.parse(researchResult.concepts[0].summary)

// Prior plan summary (if genuinely needed)
const summaryResult = await megamemory.understand({query: `phase ${phaseId} plan ${planId} summary`})
...
const result = await megamemory.understand({query: "phase 01 context"})

if (result.concepts.length === 0) {
  // Concept doesn't exist - use defaults or skip
  console.log("No context found for phase 01, proceeding without it")
} else {
...
<output>
After completion, create or update the MegaMemory phase-summary concept for this plan.
</output>
```
...
**Wave is pre-computed:** Wave numbers are assigned during `/fuska-plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
megamemory:understand({query: "phase 03-features plan 01 summary"})  # Just because it's earlier
megamemory:understand({query: "phase 03-features plan 02 summary"})  # Reflexive chaining
</context>
```
...
<output>
After completion, create or update the MegaMemory phase-summary concept (e.g., phase-03-summary-01).
</output>
```
...
```
megamemory:understand({query: "phase 03-features plan 01 summary"})
megamemory:understand({query: "phase 03-features plan 02 summary"})
```
# Parse via .concepts[0] and JSON.parse(.summary) to extract context
</context>
...
<output>
After completion, create or update the MegaMemory phase-summary concept (e.g., phase-03-summary-03).
</output>
```
