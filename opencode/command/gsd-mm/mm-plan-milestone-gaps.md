---
name: gsd-mm-plan-milestone-gaps
...
<objective>
Create all phases necessary to close gaps identified by `/gsd-audit-milestone` using MegaMemory.
...
One command creates all fix phases — no manual `/gsd-mm-add-phase` per gap.
</objective>
...
→ Suggest: "Run /gsd-audit-milestone first to create audit results"
→ Stop
...
→ Suggest: "Run /gsd-audit-milestone first"
→ Stop
...
/gsd-mm-plan-phase ${newPhases[0].number}
...
**Also available:**
- /gsd-mm-execute-phase ${newPhases[0].number} — if plans already exist
...
- [ ] User knows to run `/gsd-mm-plan-phase` next
</success_criteria>
