<purpose>
Validate built features through conversational testing with persistent state. Creates UAT concept in MegaMemory that tracks test progress, survives /new, and feeds gaps into /gsd-plan-phase --gaps.
...
Store UAT session data in MegaMemory as a concept with kind="feature" and parent_id linking to the phase concept.

**JSON Schema for concept.summary:**
```json
{
  ...
  "tests": [
    {
      ...
      "reported": "verbatim user response (if issue)",
...
Use `megamemory:understand` or directly read concept.summary and parse JSON with jq or similar.
