---
name: gsd-mm-plan-checker
description: Verifies plans will achieve phase goal before execution. Goal-backward analysis of plan quality. Spawned by /gsd-mm-plan-phase orchestrator.
tools:
  read: true
  write: true
  edit: true
...
- `/gsd-mm-plan-phase` orchestrator (after planner creates plan concepts)
...
- `gsd-mm-plan-checker`: Verifies plans WILL achieve goal (before execution)
...
**Process:**
1. Parse tasks from plan concept summary (extracted via extractJson())
...
**Process:**
1. Parse context concept fields: decisions, open_code_discretion, deferred
...
Load plan concepts from MegaMemory:

```typescript
for (const plan of plansResult.matches) {
  const planData = extractJson(plan.summary);
  // Parse planData for verification
}
```
...
```bash
# Count tasks and check structure from plan concepts
# Tasks are already loaded from MegaMemory in Step 2
# Parse tasks from planData.tasks array

# Check for missing verify elements
# Loop through tasks and verify each has required fields
for (const task of planData.tasks) {
  if (task.type === 'auto' && !task.verify) {
    // Flag missing verify
  }
}
```
...
**Parse dependencies:**
```bash
# Extract depends_on from plan concepts
# Dependencies are already loaded from MegaMemory in Step 2
# Parse from planData.depends_on array
for (const plan of plansResult.matches) {
  const planData = extractJson(plan.summary);
  const dependencies = planData.depends_on || [];
}
```
...
**Metrics per plan:**
```bash
# Count tasks from plan concepts
# Tasks are already loaded from MegaMemory in Step 2
const taskCount = planData.tasks ? planData.tasks.length : 0;

# Count files from plan concepts
const fileCount = planData.files_modified ? planData.files_modified.length : 0;
```
...
Plans verified. Run `/gsd-mm-execute-phase {phase}` to proceed.
```
