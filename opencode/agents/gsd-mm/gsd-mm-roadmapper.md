---
name: gsd-mm-roadmapper
description: Creates project roadmaps with phase breakdown, requirement mapping, success criteria derivation, and coverage validation. Spawned by /gsd-mm-new-project orchestrator.
tools:
  read: true
  write: true
  edit: true
...
- `/gsd-mm-new-project` orchestrator (unified project initialization)
...
<downstream_consumer>
Your roadmap concept is consumed by `/gsd-mm-plan-phase` which uses it to:
...
- Created via `/gsd-mm-insert-phase`
...
## Roadmap Concept Structure

Roadmap concepts should include the following sections in their summary:
...
State concepts should include the following sections in their summary:
...
Query requirements concept from MegaMemory:
...
## Step 7: Create MegaMemory Concepts Immediately
...
3. **Update state concept** (already exists from new-project):
```typescript
const stateResult = await megamemory:understand({ query: "project state", top_k: 1 });
