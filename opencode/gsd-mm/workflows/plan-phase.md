---
name: gsd-plan-phase-megamemory
...
## MegaMemory Operations for Plan Phase

### Loading Phase Context
```bash
# Load phase-specific context
PHASE_CONTEXT=$(megamemory understand "phase {N} context" top_k=5)
```

### Loading Project State
```bash
# Load overall project state
STATE=$(megamemory understand "state" top_k=5)

# Load roadmap
ROADMAP=$(megamemory understand "roadmap" top_k=5)

# Load requirements if they exist
REQUIREMENTS=$(megamemory understand "requirements" top_k=50)
```

### Querying Research and Plans
```bash
# Check for existing research
RESEARCH=$(megamemory understand "{PHASE}-research" top_k=5)

# Check for existing plans
PLANS=$(megamemory understand "{PHASE}-plan" top_k=20)

# Check for phase context
CONTEXT=$(megamemory understand "{PHASE}-context" top_k=5)
```

### Creating Plan Concepts
Plans are stored as MegaMemory concepts (not on disk). Each plan concept has:
- `name`: `{PHASE}-plan-{XX}` (e.g., `08-plan-01`)
- `kind`: `feature`
- `summary`: JSON string containing plan details:
  ```json
  {
    "phase": "08",
    "plan_id": "01",
  ...
  ```
- `parent_id`: ID of phase concept
- `edges`: Dependencies to other plans and patterns

### Parsing JSON from Concept Summary
```bash
# Extract JSON summary from MegaMemory results
PLAN_SUMMARY=$(echo "$PLAN_RESULTS" | jq '.summary' -r)
PLAN_JSON=$(echo "$PLAN_SUMMARY" | jq -c '.')
```

### Updating Plan Concepts
...
### Querying Patterns for Plan Reference
```bash
...
@gsd-opencode/gsd-opencode/integration/src/phase-templates.ts
@gsd-opencode/gsd-opencode/integration/src/helpers.ts
...
**If not found:** Error - user should run `/gsd-new-project-megamemory` first.
...
<downstream_consumer>
Output consumed by /gsd-execute-phase-megamemory
...
/gsd-execute-phase-megamemory {X}
...
- /gsd-plan-phase-megamemory {X} --research — re-research first
──────────────────────────────────────────────────────────────
```
...
- [ ] gsd-planner spawned with MegaMemory context
